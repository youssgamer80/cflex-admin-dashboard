...
const queryData = (params) => {
  return axios.get("http://192.168.1.8:4001/api/typezone", {
    params,
  });
};
...
export default defineComponent({
  ...
  setup() {
    ...
    const onDelete = (id) => {
      return axios
        .delete(
          `http://192.168.1.8:4001/api/typezone/deleteTypeZone/${id}`
        )
        .then((resp) => {
          if (resp.status === 200) {
    ...
    const onFinish = values => {
      ...
      return axios
        .put(`http://192.168.1.8:4001/api/typezone/updateTypeZone/${formState.id}`, {
  ...
  mounted(){
     ...
     fetch("http://192.168.1.8:4001/api/typezone")

<template>

  <div class="components-page-header-demo-responsive" 
  :style="{
    padding: '24px',
    margin: '24px',
    background: '#fff',
  
    textAlign: 'center'
  
  }"
  style="border: 1px solid rgb(235, 237, 240)">
    <a-page-header title="Gestion des demandes" sub-title="Détails sur demande" @back="() => $router.go(-1)">
      <template #extra>
        <a-button key="3">Regeter la demande</a-button>
        <a-button key="2">Mettre en attente</a-button>
        <a-button key="1" type="primary">Valider la demande</a-button>
      </template>
      <template #footer>
        <a-tabs>
          <a-tab-pane key="1" tab="Details" />
          <a-tab-pane key="2" tab="Rule" />
        </a-tabs>
      </template>
      <div class="content">
        <div class="main">
          <a-descriptions size="small" :column="2">
            <a-descriptions-item label="Demande de">Lili Qu</a-descriptions-item>
            <a-descriptions-item label="Immatriculation">
              <a>421421</a>
            </a-descriptions-item>
            <a-descriptions-item label="Date de demande">2017-01-10</a-descriptions-item>
            <a-descriptions-item label="Effective Time">2017-10-10</a-descriptions-item>
            <a-descriptions-item label="Mosel">
              AMG500
            </a-descriptions-item>
             <a-descriptions-item label="Marque">
              Mercedes
            </a-descriptions-item>
            <a-descriptions-item label="Type transport">
              Warren
            </a-descriptions-item>
            <a-descriptions-item label="Zone">
              Cocody Angré
            </a-descriptions-item>
          </a-descriptions>
        </div>
            <br>
            <br>
        <div class="extra">
          <div
            :style="{
              display: 'flex',
              width: 'max-content',
              justifyContent: 'flex-end',
            }"
          >
            <a-statistic
              title="Status"
              value="Pending"
              :style="{
                marginRight: '32px',
              }"
            />
            <a-statistic title="Price" prefix="$" :value="568.08" />
          </div>
        </div>
      </div>
    </a-page-header>
  </div>
</template>

<style>
.components-page-header-demo-responsive {
  padding-bottom: 20px;
}
.components-page-header-demo-responsive tr:last-child td {
  padding-bottom: 0;
}
#components-page-header-demo-responsive .content {
  display: flex;
}
#components-page-header-demo-responsive .ant-statistic-content {
  font-size: 20px;
  line-height: 28px;
}
@media (max-width: 576px) {
  #components-page-header-demo-responsive .content {
    display: block;
  }

  #components-page-header-demo-responsive .main {
    width: 100%;
    margin-bottom: 12px;
  }

  #components-page-header-demo-responsive .extra {
    width: 100%;
    margin-left: 0;
    text-align: left;
  }
}
</style>
...
const queryData = (params) => {
  return axios.get("http://192.168.252.135:4001/api/pointarrets", {
    params,
  });
};
...
export default defineComponent({
  ...
  setup() {
    ...
    const onSubmit = async () => {
      const resp = await axios.put(
        `http://192.168.252.135:4001/api/pointarrets/updatePointArret/${formState.id}`,
        {
          nom: formState.nom,
    ...
    const onDelete = async (id) => {
      const resp = await axios.delete(
        `http://192.168.252.135:4001/api/pointarrets/deletePointArret/${id}`
      );
      if (resp.status === 200) {
  ...
  mounted() {
    ...
    fetch("http://192.168.252.135:4001/api/zones")
      .then((response) => response.json())
      .then((res) => {
    ...
    fetch(
      "http://192.168.252.135:4001/api/v1/TypePointArret/getTypePointArrets"
    )
      .then((response) => response.json())
      .then((res) => {
        this.dataListTypePointArret = res.data;
